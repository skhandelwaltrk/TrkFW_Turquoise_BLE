...
int sendDataUrlToCloud(const char *packetDataBuff, size_t packetDataLen) {
    ...
    TRK_PRINTF1("Curl_Proc %d: curl rqst going=%s", ++curlProcessCount, urlBuff);
    ...
    if (!initialized) {
        ...
        if (err_ret != 0) {
            TRK_PRINTF1("Curl_Proc: failed curl_global_init");
            return 1;
        }
        ...
        if (!curl) {
            curl_global_cleanup();
            TRK_PRINTF1("Curl_Proc: failed curl_easy_init");
            return 1;
        }
    ...
    if (err_ret != 0) {
        TRK_PRINTF1("Curl_Proc: failed set CURLOPT_URL");
    ...
    if (err_ret != 0) {
        TRK_PRINTF1("Curl_Proc: failed set CURLOPT_TIMEOUT");
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        initialized = false;
        curl = nullptr;
    ...
    if (err_ret != 0) {
        TRK_PRINTF1("Curl_Proc: failed set CURLOPT_WRITEDATA");
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        initialized = false;
        curl = nullptr;
    ...
    // Check for errors
    if (res != CURLE_OK) {
        TRK_PRINTF1("Curl_Proc: curl_easy_perform() failed: %s", curl_easy_strerror(res));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        initialized = false;
        curl = nullptr;
    ...
    TRK_PRINTF1("Curl_Proc %d: Response data: %s, Total transfer time: %.3f seconds", 
